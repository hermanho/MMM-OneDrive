...
/*! @azure/msal-node v6.0.1 2026-09-15 */ class Serializer {
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const RETURN_SPA_CODE = "return_spa_code", X_CLIENT_EXTRA_SKU = "x-client-xtra-sku", IMDS_ENDPOINT = "http://169.254.169.254/metadata/instance/compute", KNOWN_PUBLIC_CLOUDS = [ "login.microsoftonline.com", "login.windows.net", "login.microsoft.com", "sts.windows.net" ], OIDC_DEFAULT_SCOPES = [ "openid", "profile", "offline_access" ], OIDC_SCOPES = [ ...OIDC_DEFAULT_SCOPES, "email" ], HeaderNames_CONTENT_TYPE = "Content-Type", HeaderNames_CONTENT_LENGTH = "Content-Length", HeaderNames_DPOP = "DPoP", HeaderNames_RETRY_AFTER = "Retry-After", HeaderNames_CCS_HEADER = "X-AnchorMailbox", HeaderNames_X_MS_REQUEST_ID = "x-ms-request-id", HeaderNames_X_MS_HTTP_VERSION = "x-ms-httpver", AADAuthority_COMMON = "common", AADAuthority_ORGANIZATIONS = "organizations", AADAuthority_CONSUMERS = "consumers", ClaimsRequestKeys_ACCESS_TOKEN = "access_token", ClaimsRequestKeys_XMS_CC = "xms_cc", ClaimsRequestKeys_ID_TOKEN = "id_token", ClaimsRequestKeys_SIGNIN_STATE = "signin_state", ClaimsRequestKeys_LOGIN_HINT = "login_hint", ClaimsRequestKeys_TENANT_REGION_SUB_SCOPE = "tenant_region_sub_scope", PromptValue_SELECT_ACCOUNT = "select_account", PromptValue_NONE = "none", CodeChallengeMethodValues_S256 = "S256", OAuthResponseType_CODE = "code", OAuthResponseType_IDTOKEN_TOKEN = "id_token token", ResponseMode_QUERY = "query", ResponseMode_FORM_POST = "form_post", GrantType_AUTHORIZATION_CODE_GRANT = "authorization_code", GrantType_RESOURCE_OWNER_PASSWORD_GRANT = "password", GrantType_REFRESH_TOKEN_GRANT = "refresh_token", GrantType_DEVICE_CODE_GRANT = "device_code", CredentialType = {
  ID_TOKEN: "IdToken",
...
class AuthError extends Error {
  constructor(errorCode, correlationId, errorMessage, suberror) {
    ...
    this.correlationId = correlationId, this.name = "AuthError";
  }
}
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ class ClientAuthError extends AuthError {
  constructor(errorCode, correlationId, additionalMessage) {
    super(errorCode, correlationId, additionalMessage), this.name = "ClientAuthError", 
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const methodNotImplemented = "method_not_implemented";
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ function extractTokenClaims(encodedToken, base64Decode, correlationId) {
  const jswPayload = function(authToken, correlationId) {
    if (!authToken) {
      throw createClientAuthError("null_or_empty_token", correlationId);
    }
    const matches = /^([^\.\s]*)\.([^\.\s]+)\.([^\.\s]*)$/.exec(authToken);
    if (!matches || matches.length < 4) {
      throw createClientAuthError("token_parsing_error", correlationId);
    }
    return matches[2];
  }
  /*! @azure/msal-common v16.14.1 2026-09-15 */ (encodedToken, correlationId);
  try {
...
function updateAccountTenantProfileData(baseAccountInfo, tenantProfile, idTokenClaims, idTokenSecret) {
  ...
  if (idTokenClaims) {
    const {isHomeTenant: isHomeTenant, ...claimsSourcedTenantProfile} = buildTenantProfile(baseAccountInfo.homeAccountId, baseAccountInfo.localAccountId, baseAccountInfo.tenantId, updatedAccountInfo.nativeAccountId, idTokenClaims);
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const AuthorityType_Default = 0, AuthorityType_Adfs = 1, AuthorityType_Ciam = 3;

/*! @azure/msal-common v16.14.1 2026-09-15 */ function getTenantIdFromIdTokenClaims(idTokenClaims) {
  if (idTokenClaims) {
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const ProtocolMode_AAD = "AAD", ProtocolMode_OIDC = "OIDC";

/*! @azure/msal-common v16.14.1 2026-09-15 */ function getAccountInfo(accountEntity) {
  const tenantProfiles = accountEntity.tenantProfiles || [];
  0 === tenantProfiles.length && accountEntity.realm && accountEntity.localAccountId && tenantProfiles.push(buildTenantProfile(accountEntity.homeAccountId, accountEntity.localAccountId, accountEntity.realm, accountEntity.nativeAccountId));
  const homeTenantProfile = tenantProfiles.find(tp => tp.tenantId === accountEntity.realm), nativeAccountId = homeTenantProfile?.nativeAccountId || accountEntity.nativeAccountId;
  return {
...
class ClientConfigurationError extends AuthError {
  constructor(errorCode, correlationId) {
    super(errorCode, correlationId), this.name = "ClientConfigurationError", Object.setPrototypeOf(this, ClientConfigurationError.prototype);
  }
}
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ class UrlString {
  get urlString() {
    return this._urlString;
  }
  constructor(url, correlationId) {
    if (this._urlString = url, this.correlationId = correlationId, !this._urlString) {
      throw createClientConfigurationError("empty_url_error", correlationId);
    }
  ...
  validateAsUri() {
    ...
    try {
      ...
      throw createClientConfigurationError("url_parse_error", this.correlationId);
    }
    if (!components.HostNameAndPort || !components.PathSegments) {
      throw createClientConfigurationError("url_parse_error", this.correlationId);
    }
    if (!components.Protocol || "https:" !== components.Protocol.toLowerCase()) {
      throw createClientConfigurationError("authority_uri_insecure", this.correlationId);
    }
  }
  ...
  replaceTenantPath(tenantId) {
    ...
    UrlString.constructAuthorityUriFromObject(urlObject, this.correlationId);
  }
  getUrlComponents() {
  ...
  static getDomainFromUrl(url, correlationId) {
    ...
    if (!match) {
      throw createClientConfigurationError("url_parse_error", correlationId);
    }
    return match[2];
  }
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const rawMetdataJSON = {
  endpointMetadata: [ {
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ InstanceDiscoveryMetadata.metadata.forEach(metadataEntry => {
  metadataEntry.aliases.forEach(alias => {
    InstanceDiscoveryMetadataAliases.add(alias);
  });
});

const AzureCloudInstance_None = "none";

/*! @azure/msal-common v16.14.1 2026-09-15 */
/*! @azure/msal-common v16.14.1 2026-09-15 */
const invokeAsync = (callback, eventName, logger, telemetryClient, correlationId) => (...args) => {
...
class RegionDiscovery {
  ...
  async detectRegion(environmentRegion, regionDiscoveryMetadata) {
    ...
    if (autodetectedRegionName) {
      ...
      try {
        const localIMDSVersionResponse = await invokeAsync(this.getRegionFromIMDS.bind(this), "regionDiscoveryGetRegionFromIMDS", this.logger, this.performanceClient, this.correlationId)("2021-02-01", options);
        if (200 === localIMDSVersionResponse.status && (autodetectedRegionName = localIMDSVersionResponse.body?.location, 
        autodetectedRegionName && (regionDiscoveryMetadata.region_source = RegionDiscoverySources_IMDS)), 
        400 === localIMDSVersionResponse.status) {
          ...
          200 === currentIMDSVersionResponse.status && (autodetectedRegionName = currentIMDSVersionResponse.body?.location, 
          autodetectedRegionName && (regionDiscoveryMetadata.region_source = RegionDiscoverySources_IMDS));
        }
      } catch (e) {
        return regionDiscoveryMetadata.region_source = RegionDiscoverySources_FAILED_AUTO_DETECTION, 
  ...
  async getRegionFromIMDS(version, options) {
    return this.networkInterface.sendGetRequestAsync(`${IMDS_ENDPOINT}?api-version=${version}`, options, 2e3);
  }
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ function isCredentialEntity(entity) {
...
function serializeAttributeTokens(attributeTokens) {
  if (attributeTokens && 0 !== attributeTokens.length) {
    return [ ...attributeTokens ].sort().join(" ");
  }
}

/*! @azure/msal-common v16.14.1 2026-09-15 */ RegionDiscovery.IMDS_OPTIONS = {
  headers: {
    Metadata: "true"
  }
};
...
class Authority {
  ...
  getAuthorityType(authorityUri) {
    ...
    return pathSegments.length && "adfs" === pathSegments[0].toLowerCase() ? AuthorityType_Adfs : AuthorityType_Default;
  }
  get authorityType() {
  ...
  set canonicalAuthority(url) {
    this._canonicalAuthority = new UrlString(url, this.correlationId), this._canonicalAuthority.validateAsUri(), 
  ...
  get authorizationEndpoint() {
    ...
    throw createClientAuthError("endpoints_resolution_error", this.correlationId);
  }
  get tokenEndpoint() {
    if (this.discoveryComplete()) {
  ...
  get endSessionEndpoint() {
    if (this.discoveryComplete()) {
      if (!this.metadata.end_session_endpoint) {
        throw createClientAuthError("end_session_endpoint_not_supported", this.correlationId);
      }
  ...
  replacePath(urlString) {
    ...
    const cachedAuthorityUrlComponents = new UrlString(this.metadata.canonical_authority, this.correlationId).getUrlComponents(), cachedAuthorityParts = cachedAuthorityUrlComponents.PathSegments;
  ...
  async updateEndpointMetadata(metadataEntity) {
    ...
    throw createClientAuthError("openid_config_error", this.defaultOpenIdConfigurationEndpoint, this.correlationId);
  }
  ...
  getEndpointMetadataFromConfig() {
    if (this.authorityOptions.authorityMetadata) {
      try {
        ...
        throw createClientConfigurationError("invalid_authority_metadata", this.correlationId);
      }
    }
    return null;
  }
  async getEndpointMetadataFromNetwork() {
    ...
    try {
      const response = await this.networkInterface.sendGetRequestAsync(openIdConfigurationEndpoint, options), isValidResponse = 
      /*! @azure/msal-common v16.14.1 2026-09-15 */
      function(response) {
  ...
  async updateMetadataWithRegionalInformation(metadata) {
    ...
    if (userConfiguredAzureRegion) {
      if ("TryAutoDetect" !== userConfiguredAzureRegion) {
        ...
        this.regionDiscoveryMetadata.region_used = userConfiguredAzureRegion, Authority.replaceWithRegionalInformation(metadata, userConfiguredAzureRegion, this.correlationId);
      }
      ...
      if (autodetectedRegionName) {
        ...
        this.regionDiscoveryMetadata.region_used = autodetectedRegionName, Authority.replaceWithRegionalInformation(metadata, autodetectedRegionName, this.correlationId);
      }
  ...
  async updateCloudDiscoveryMetadata(metadataEntity) {
    ...
    throw createClientConfigurationError("untrusted_authority", this.correlationId);
  }
  ...
  getCloudDiscoveryMetadataFromConfig() {
    ...
    if (this.authorityOptions.cloudDiscoveryMetadata) {
      ...
      try {
        ...
        createClientConfigurationError("invalid_cloud_discovery_metadata", this.correlationId);
      }
    }
    return this.isInKnownAuthorities(this.hostnameAndPort) ? (this.logger.verbose("The host is included in knownAuthorities. Creating new cloud discovery metadata from the host.", this.correlationId), 
  ...
  async getCloudDiscoveryMetadataFromNetwork() {
    ...
    try {
      ...
      /*! @azure/msal-common v16.14.1 2026-09-15 */ (response.body)) {
  ...
  isInKnownAuthorities(host) {
    ...
    return this.authorityOptions.knownAuthorities.filter(authority => authority && UrlString.getDomainFromUrl(authority, this.correlationId).toLowerCase() === normalizedHost).length > 0;
  }
  ...
  validateIssuer(issuer) {
    if (!issuer) {
      throw createClientConfigurationError("issuer_validation_failed", this.correlationId);
    }
    let issuerUrl;
    try {
  ...
  static replaceWithRegionalInformation(metadata, azureRegion, correlationId) {
    ...
    return regionalMetadata.authorization_endpoint = Authority.buildRegionalAuthorityString(regionalMetadata.authorization_endpoint, azureRegion, correlationId), 
    ...
    regionalMetadata.end_session_endpoint && (regionalMetadata.end_session_endpoint = Authority.buildRegionalAuthorityString(regionalMetadata.end_session_endpoint, azureRegion, correlationId)), 
    regionalMetadata;
  }
...
async function createDiscoveredInstance(authorityUri, networkClient, cacheManager, authorityOptions, logger, correlationId, performanceClient) {
  const authorityUriFinal = Authority.transformCIAMAuthority(formatAuthorityUri(authorityUri), correlationId), acquireTokenAuthority = new Authority(authorityUriFinal, networkClient, cacheManager, authorityOptions, logger, correlationId, performanceClient);
  try {
    ...
    throw createClientAuthError("endpoints_resolution_error", correlationId);
  }
}

/*! @azure/msal-common v16.14.1 2026-09-15 */ Authority.reservedTenantDomains = new Set([ "{tenant}", "{tenantid}", AADAuthority_COMMON, AADAuthority_CONSUMERS, AADAuthority_ORGANIZATIONS ]);
...
class ScopeSet {
  constructor(inputScopes, correlationId) {
    ...
    if (!filteredInput || !filteredInput.length) {
      throw createClientConfigurationError("empty_input_scopes_error", correlationId);
    }
  ...
  static createSearchScopes(inputScopeString, correlationId) {
    const scopesToUse = inputScopeString && inputScopeString.length > 0 ? inputScopeString : [ ...OIDC_DEFAULT_SCOPES ], scopeSet = new ScopeSet(scopesToUse, correlationId);
    return scopeSet.containsOnlyOIDCScopes() ? scopeSet.removeScope("offline_access") : scopeSet.removeOIDCScopes(), 
    scopeSet;
  }
  containsScope(scope) {
    const lowerCaseScopes = this.printScopesLowerCase().split(" "), lowerCaseScopesSet = new ScopeSet(lowerCaseScopes, this.correlationId);
  ...
  appendScopes(newScopes) {
    try {
      ...
      throw createClientAuthError("cannot_append_scopeset", this.correlationId);
    }
  }
  removeScope(scope) {
    if (!scope) {
      throw createClientAuthError("cannot_remove_empty_scope", this.correlationId);
    }
  ...
  unionScopeSets(otherScopes) {
    if (!otherScopes) {
      throw createClientAuthError("empty_input_scopeset", this.correlationId);
    }
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ function instrumentBrokerParams(parameters, correlationId, performanceClient) {
  if (!correlationId) {
    return;
  }
...
function addScopes(parameters, scopes, correlationId, addOidcScopes = !0, defaultScopes = OIDC_DEFAULT_SCOPES) {
  !addOidcScopes || defaultScopes.includes("openid") || scopes.includes("openid") || defaultScopes.push("openid");
  const requestScopes = addOidcScopes ? [ ...scopes || [], ...defaultScopes ] : scopes || [], scopeSet = new ScopeSet(requestScopes, correlationId);
...
function addClaims(parameters, correlationId, claims, clientCapabilities, skipBrokerClaims, claimsToMerge) {
  const mergedClaims = function(claims, clientCapabilities, correlationId = "") {
    let mergedClaims = claims ? function(claims, correlationId = "") {
      let parsed;
      try {
        parsed = JSON.parse(claims);
      } catch (e) {
        throw createClientConfigurationError("invalid_claims", correlationId);
      }
      if (value = parsed, "object" != typeof value || null === value || Array.isArray(value)) {
        throw createClientConfigurationError("invalid_claims", correlationId);
      }
      var value;
      return parsed;
    }(claims, correlationId) : {};
    Object.prototype.hasOwnProperty.call(mergedClaims, ClaimsRequestKeys_ID_TOKEN) || (mergedClaims[ClaimsRequestKeys_ID_TOKEN] = {});
    const idTokenClaims = mergedClaims[ClaimsRequestKeys_ID_TOKEN];
    for (const [key, value] of Object.entries(DEFAULT_ID_TOKEN_CLAIMS)) {
      key in idTokenClaims || (idTokenClaims[key] = value);
    }
    clientCapabilities && clientCapabilities.length > 0 && (Object.prototype.hasOwnProperty.call(mergedClaims, ClaimsRequestKeys_ACCESS_TOKEN) || (mergedClaims[ClaimsRequestKeys_ACCESS_TOKEN] = {}), 
    mergedClaims[ClaimsRequestKeys_ACCESS_TOKEN][ClaimsRequestKeys_XMS_CC] = {
      values: clientCapabilities
    });
    return JSON.stringify(mergedClaims);
  }(claims, skipBrokerClaims && parameters.has("brk_client_id") ? void 0 : clientCapabilities, correlationId);
...
const DEFAULT_ID_TOKEN_CLAIMS = {
  [ClaimsRequestKeys_SIGNIN_STATE]: {
    essential: !1
  },
  [ClaimsRequestKeys_LOGIN_HINT]: {
    essential: !1
  },
  [ClaimsRequestKeys_TENANT_REGION_SUB_SCOPE]: {
    essential: !1
  }
};
...
function addAttributeTokens(parameters, attributeTokens) {
  ...
  serialized ? parameters.set("attribute_tokens", serialized) : parameters.delete("attribute_tokens");
}

/*! @azure/msal-common v16.14.1 2026-09-15 */ function getDeserializedResponse(responseString) {
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const JsonWebTokenAlgorithms_ES256 = "ES256", JsonWebTokenAlgorithms_RS256 = "RS256", DEFAULT_CRYPTO_IMPLEMENTATION = {
  createNewGuid: () => {
...
function addLogToCache(correlationId, loggedMessage) {
  ...
  if (data) {
    ...
    void 0 !== firstKey && correlationCache.delete(firstKey);
  }
  data.logs.push({
    ...loggedMessage,
...
class Logger {
  ...
  logMessage(logMessage, options) {
    const correlationId = options.correlationId, messageHash = function(str) {
      if (str.length < 6) {
        return null;
      }
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const name$1 = "@azure/msal-common";
...
class CacheManager {
  constructor(clientId, cryptoImpl, logger, performanceClient, staticAuthorityOptions, tokenBindingKeyManager) {
    this.clientId = clientId, this.cryptoImpl = cryptoImpl, this.tokenBindingKeyManager = tokenBindingKeyManager, 
    this.commonLogger = logger.clone(name$1, "16.14.1"), this.staticAuthorityOptions = staticAuthorityOptions, 
  ...
  getTenantedAccountInfoByFilter(accountInfo, tokenKeys, tenantProfile, correlationId, tenantProfileFilter) {
    ...
    return idToken && (idTokenClaims = extractTokenClaims(idToken.secret, this.cryptoImpl.base64Decode, correlationId), 
  ...
  async saveCacheRecord(cacheRecord, correlationId, kmsi, apiId, storeInCache) {
    if (!cacheRecord) {
      throw createClientAuthError("invalid_cache_record", correlationId);
    }
    try {
  ...
  async saveAccessToken(credential, correlationId, kmsi) {
    ...
    credential.additionalCacheKeyComponents && Object.keys(credential.additionalCacheKeyComponents).length > 0 && (additionalCacheKeyHash = await this.cryptoImpl.hashString(JSON.stringify(credential.additionalCacheKeyComponents)));
    ...
    }, tokenKeys = this.getTokenKeys(), currentScopes = ScopeSet.fromString(credential.target, correlationId);
    tokenKeys.accessToken.forEach(key => {
      ...
      if (tokenEntity && this.credentialMatchesFilter(tokenEntity, accessTokenFilter, correlationId)) {
        ScopeSet.fromString(tokenEntity.target, correlationId).intersectingScopeSets(currentScopes) && this.removeAccessToken(key, correlationId);
      }
  ...
  credentialMatchesFilter(entity, filter, correlationId) {
    ...
    if (entity.credentialType === CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME && !this.matchAccessTokenWithAuthScheme(entity, filter)) {
      return !1;
    }
    const entityComponents = entity.additionalCacheKeyComponents, filterComponents = filter.additionalCacheKeyComponents, entityHasComponents = !!entityComponents && Object.keys(entityComponents).length > 0, filterHasComponents = !!filterComponents && Object.keys(filterComponents).length > 0;
    if (entityHasComponents !== filterHasComponents) {
      return !1;
    }
    if (entityHasComponents && filterHasComponents) {
      const entityKeys = Object.keys(entityComponents).sort(), filterKeys = Object.keys(filterComponents).sort();
  ...
  removeAccessToken(key, correlationId) {
    ...
    }, correlationId), credential.credentialType.toLowerCase() === CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME.toLowerCase())) {
       ...
       case AuthenticationScheme.DPOP.toLowerCase():
        {
          ...
          kid && this.tokenBindingKeyManager.removeTokenBindingKey(kid, correlationId).catch(() => {
            this.commonLogger.error("Failed to remove token binding key", correlationId), this.performanceClient?.incrementFields({
  ...
  getAccessToken(account, request, tokenKeys, targetRealm) {
    ...
    const scopes = ScopeSet.createSearchScopes(request.scopes, correlationId), authScheme = request.authenticationScheme || AuthenticationScheme.BEARER, credentialType = authScheme.toLowerCase() !== AuthenticationScheme.BEARER.toLowerCase() ? CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME : CredentialType.ACCESS_TOKEN, attributeTokenPartition = serializeAttributeTokens(request.attributeTokens), additionalCacheKeyComponents = attributeTokenPartition ? {
      attribute_tokens: attributeTokenPartition
    } : void 0, accessTokenFilter = {
      homeAccountId: account.homeAccountId,
      environment: account.environment,
    ...
    }, accessTokenKeys = tokenKeys && tokenKeys.accessToken || this.getTokenKeys().accessToken, accessTokens = [], matchedKeys = [];
    return accessTokenKeys.forEach(key => {
    ...
    }), accessTokens.length < 1 ? (this.commonLogger.info("CacheManager:getAccessToken - No token found", correlationId), 
    null) : accessTokens.length > 1 ? (this.commonLogger.info("CacheManager:getAccessToken - Multiple access tokens found, clearing them", correlationId), 
  ...
  readAppMetadataFromCache(environment, correlationId) {
    ...
    if (numAppMetadata > 1) {
      throw createClientAuthError("multiple_matching_appMetadata", correlationId);
    }
  ...
  matchKeyBoundAccessToken(entity, filter) {
    return !filter.keyId || this.matchKeyId(entity, filter.keyId);
  }
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const PerformanceEventStatus_InProgress = 1;

/*! @azure/msal-common v16.14.1 2026-09-15 */ class StubPerformanceClient {
  generateId() {
    return "callback-id";
  }
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const DEFAULT_SYSTEM_OPTIONS$1 = {
...
function buildClientConfiguration({authOptions: userAuthOptions, systemOptions: userSystemOptions, loggerOptions: userLoggerOption, storageInterface: storageImplementation, networkInterface: networkImplementation, cryptoInterface: cryptoImplementation, tokenBindingKeyManager: tokenBindingKeyManager, clientCredentials: clientCredentials, libraryInfo: libraryInfo, telemetry: telemetry, serverTelemetryManager: serverTelemetryManager, persistencePlugin: persistencePlugin, serializableCache: serializableCache}) {
  const loggerOptions = {
    ...DEFAULT_LOGGER_IMPLEMENTATION,
    ...userLoggerOption
  }, resolvedTokenBindingKeyManager = tokenBindingKeyManager || DEFAULT_TOKEN_BINDING_KEY_MANAGER;
  return {
    ...
    storageInterface: storageImplementation || new DefaultStorageClass(userAuthOptions.clientId, DEFAULT_CRYPTO_IMPLEMENTATION, new Logger(loggerOptions, name$1, "16.14.1"), new StubPerformanceClient, void 0, resolvedTokenBindingKeyManager),
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ class TokenCacheContext {
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ class JoseHeaderError extends AuthError {
  constructor(errorCode, correlationId, errorMessage) {
    super(errorCode, correlationId, errorMessage), this.name = "JoseHeaderError", Object.setPrototypeOf(this, JoseHeaderError.prototype);
  }
}
...
class JoseHeader {
  constructor(options, correlationId) {
    if ("string" != typeof options.alg || !options.alg) {
      throw createJoseHeaderError("missing_alg_error", correlationId);
    }
    this.typ = options.typ, this.alg = options.alg, this.kid = options.kid, this.jwk = options.jwk;
  }
  static getShrHeader(shrHeaderOptions, correlationId) {
    if (!shrHeaderOptions.kid) {
      throw createJoseHeaderError("missing_kid_error", correlationId);
    }
    if (!shrHeaderOptions.alg) {
      throw createJoseHeaderError("missing_alg_error", correlationId);
    }
    return new JoseHeader({
      typ: shrHeaderOptions.typ || JsonWebTokenTypes_Pop,
  ...
  static getDpopHeader(dpopHeaderOptions, correlationId) {
    if (!function(value) {
      if ("object" != typeof value || null === value || "[object Object]" !== Object.prototype.toString.call(value)) {
        return !1;
      }
    ...
    /*! @azure/msal-common v16.14.1 2026-09-15 */ (dpopHeaderOptions.jwk)) {
      throw createJoseHeaderError("missing_jwk_error", correlationId);
    }
    if (!dpopHeaderOptions.alg) {
      throw createJoseHeaderError("missing_alg_error", correlationId);
    }
    if (0 === Object.keys(dpopHeaderOptions.jwk).length) {
      throw createJoseHeaderError("invalid_jwk_error", correlationId);
    }
    return new JoseHeader({
      typ: JsonWebTokenTypes_Dpop,
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const KeyLocation_SW = "sw", SHR_TOKEN_BINDING_KEY_ALGORITHM = JsonWebTokenAlgorithms_RS256;

class PopTokenGenerator {
  constructor(cryptoUtils, tokenBindingKeyManager, performanceClient) {
    this.cryptoUtils = cryptoUtils, this.tokenBindingKeyManager = tokenBindingKeyManager, 
    this.performanceClient = performanceClient;
  }
  ...
  async signPayload(payload, keyId, request, claims) {
    const {resourceRequestMethod: resourceRequestMethod, resourceRequestUri: resourceRequestUri, shrClaims: shrClaims, shrNonce: shrNonce, shrOptions: shrOptions} = request, resourceUrlString = resourceRequestUri ? new UrlString(resourceRequestUri, request.correlationId) : void 0, resourceUrlComponents = resourceUrlString?.getUrlComponents(), publicKeyJwk = await this.tokenBindingKeyManager.getTokenBindingPublicKeyJwk(keyId, request.correlationId), encodedKeyIdThumbprint = this.cryptoUtils.base64UrlEncode(JSON.stringify({
      kid: keyId
    })), shrAlgorithm = shrOptions?.header?.alg || publicKeyJwk.alg || SHR_TOKEN_BINDING_KEY_ALGORITHM, shrHeader = JoseHeader.getShrHeader({
      ...shrOptions?.header,
      alg: shrAlgorithm,
      kid: encodedKeyIdThumbprint
    }, request.correlationId), shrPayload = {
      at: payload,
      ts: nowSeconds(),
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ const DPOP_HTM_REGEX = /^[!#$%&'*+\-.^_`|~0-9A-Za-z]+$/, DPOP_JWT_HEADER_ALGORITHM = JsonWebTokenAlgorithms_ES256;

function normalizeHtm(htm, correlationId) {
  if ("string" != typeof htm || !DPOP_HTM_REGEX.test(htm)) {
    throw createClientConfigurationError("invalid_dpop_htm", correlationId);
  }
...
function normalizeHtu(url, correlationId) {
  ...
  if (!/^https:\/\//i.test(url) || "https:" !== parsedUrl.protocol || parsedUrl.username || parsedUrl.password) {
    throw createClientConfigurationError("invalid_dpop_htu", correlationId);
  }
  return parsedUrl.search = "", parsedUrl.hash = "", parsedUrl.href;
}

function validateDpopNonce(nonce, correlationId) {
  if (void 0 !== nonce && 0 === nonce.trim().length) {
    throw createClientConfigurationError("invalid_dpop_nonce", correlationId);
  }
}
...
class DpopProofGenerator {
  ...
  buildTokenProofClaims(params, correlationId = "") {
    ...
    return void 0 !== params.nonce && (claims.nonce = params.nonce), claims;
  }
  ...
  async generateResourceProof(params, keyId, correlationId = "") {
    const {htu: htu, htm: htm, nonce: nonce} = params;
    if (!htu || !htm) {
      throw createClientConfigurationError("dpop_missing_resource_context", correlationId);
    }
  ...
  async generateProof(claims, keyId, correlationId) {
    ...
    return this.cryptoUtils.signTokenBindingJwt(function(publicJwk, correlationId) {
      return JoseHeader.getDpopHeader({
        alg: DPOP_JWT_HEADER_ALGORITHM,
        jwk: publicJwk
      }, correlationId);
    }(publicJwk, correlationId), claims, keyId, correlationId);
  }
}

/*! @azure/msal-common v16.14.1 2026-09-15 */ const InteractionRequiredServerErrorMessage = [ "interaction_required", "consent_required", "login_required", "bad_token", "ui_not_allowed", "interrupted_user" ], InteractionRequiredAuthSubErrorMessage = [ "message_only", "additional_action", "basic_action", "user_password_expired", "consent_required", "bad_token", "ui_not_allowed", "interrupted_user" ];

class InteractionRequiredAuthError extends AuthError {
  constructor(errorCode, correlationId, errorMessage, subError, timestamp, traceId, claims, errorNo) {
    super(errorCode, correlationId, errorMessage, subError), Object.setPrototypeOf(this, InteractionRequiredAuthError.prototype), 
    this.timestamp = timestamp || "", this.traceId = traceId || "", this.claims = claims || "", 
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ class ServerError extends AuthError {
  constructor(errorCode, correlationId, errorMessage, subError, errorNo, status) {
    super(errorCode, correlationId, errorMessage, subError), this.name = "ServerError", 
    this.errorNo = errorNo, this.status = status, Object.setPrototypeOf(this, ServerError.prototype);
  }
}
...
class ResponseHandler {
  ...
  async handleServerTokenResponse(serverTokenResponse, authority, reqTimestamp, request, apiId, authCodePayload, userAssertionHash, handlingRefreshTokenResponse, forceCacheRefreshTokenResponse, serverRequestId, additionalCacheKeyComponents) {
    ...
    authCodePayload && Object.prototype.hasOwnProperty.call(authCodePayload, "nonce")) {
      const expectedNonce = authCodePayload.nonce, tokenNonce = idTokenClaims?.nonce;
      if (void 0 !== tokenNonce && void 0 === expectedNonce) {
        throw this.logger.warning("Authorization code response contains an ID Token nonce, but no expected nonce was supplied. Rejecting the response.", request.correlationId), 
        createClientAuthError("nonce_mismatch", request.correlationId);
      }
      if (void 0 !== expectedNonce && ("string" != typeof expectedNonce || "string" != typeof tokenNonce || expectedNonce !== tokenNonce)) {
        throw createClientAuthError("nonce_mismatch", request.correlationId);
      }
    }
    if (this.homeAccountIdentifier = function(serverClientInfo, authType, logger, cryptoObj, correlationId, idTokenClaims) {
      if (authType !== AuthorityType_Adfs) {
        if (serverClientInfo) {
          try {
    ...
    authCodePayload && authCodePayload.state && (requestStateObj = function(base64Decode, state, correlationId) {
      if (!base64Decode) {
        throw createClientAuthError("no_crypto_object", correlationId);
      }
      if (!state) {
        throw createClientAuthError("invalid_state", correlationId);
      }
      try {
    ...
    }(this.cryptoObj.base64Decode, authCodePayload.state, request.correlationId)), serverTokenResponse.key_id = serverTokenResponse.key_id || request.dpopJkt || request.sshKid || void 0, 
    request.authenticationScheme === AuthenticationScheme.DPOP) {
      if (serverTokenResponse.token_type?.toLowerCase() !== AuthenticationScheme.DPOP.toLowerCase()) {
        ...
        }, request.correlationId), createClientAuthError("dpop_token_type_mismatch", request.correlationId);
      }
      serverTokenResponse.token_type = AuthenticationScheme.DPOP;
    }
    const attributeTokenPartition = serializeAttributeTokens(request.attributeTokens), cacheKeyComponents = additionalCacheKeyComponents ?? (attributeTokenPartition ? {
      attribute_tokens: attributeTokenPartition
    } : void 0), cacheRecord = this.generateCacheRecord(serverTokenResponse, authority, reqTimestamp, request, idTokenClaims, userAssertionHash, authCodePayload, cacheKeyComponents);
    let cacheContext;
    try {
      ...
      handlingRefreshTokenResponse && !forceCacheRefreshTokenResponse && cacheRecord.account) {
        if (this.cacheStorage.getAllAccounts({
          ...
          this.performanceClient?.addFields({
            acntLoggedOut: !0
          }, request.correlationId), await ResponseHandler.generateAuthenticationResult(this.cryptoObj, authority, cacheRecord, !1, request, this.performanceClient, {
            idTokenClaims: idTokenClaims,
    ...
    return ResponseHandler.generateAuthenticationResult(this.cryptoObj, authority, cacheRecord, !1, request, this.performanceClient, {
      idTokenClaims: idTokenClaims,
      requestState: requestStateObj,
  ...
  generateCacheRecord(serverTokenResponse, authority, reqTimestamp, request, idTokenClaims, userAssertionHash, authCodePayload, additionalCacheKeyComponents) {
    ...
    if (!env) {
      throw createClientAuthError("invalid_cache_environment", request.correlationId);
    }
    ...
    tenantId = claimsTenantId || "", cachedIdToken = {
      ...
      const cachedAccount = 1 === matchedAccounts.length ? matchedAccounts[0] : null, baseAccount = cachedAccount || function(accountDetails, authority, correlationId, base64Decode) {
        ...
        if (!env) {
          throw createClientAuthError("invalid_cache_environment", correlationId);
        }
        ...
        let tenantProfiles;
        tenantProfiles = accountDetails.tenantProfiles ? accountDetails.tenantProfiles : [ buildTenantProfile(accountDetails.homeAccountId, localAccountId, realm, accountDetails.nativeAccountId, accountDetails.idTokenClaims) ];
        return {
        ...
        nativeAccountId: nativeAccountId
      }, authority, correlationId, base64Decode), tenantProfiles = baseAccount.tenantProfiles || [], tenantId = claimsTenantId || baseAccount.realm;
    ...
    /*! @azure/msal-common v16.14.1 2026-09-15 */ (this.cacheStorage, authority, this.homeAccountIdentifier, this.cryptoObj.base64Decode, request.correlationId, idTokenClaims, serverTokenResponse.client_info, env, claimsTenantId, authCodePayload, void 0, this.logger, this.performanceClient));
    let cachedAccessToken = null;
    if (serverTokenResponse.access_token) {
      const responseScopes = serverTokenResponse.scope ? ScopeSet.fromString(serverTokenResponse.scope, request.correlationId) : new ScopeSet(request.scopes || [], request.correlationId), expiresIn = ("string" == typeof serverTokenResponse.expires_in ? parseInt(serverTokenResponse.expires_in, 10) : serverTokenResponse.expires_in) || 0, extExpiresIn = ("string" == typeof serverTokenResponse.ext_expires_in ? parseInt(serverTokenResponse.ext_expires_in, 10) : serverTokenResponse.ext_expires_in) || 0, refreshIn = ("string" == typeof serverTokenResponse.refresh_in ? parseInt(serverTokenResponse.refresh_in, 10) : serverTokenResponse.refresh_in) || void 0, tokenExpirationSeconds = reqTimestamp + expiresIn, extendedTokenExpirationSeconds = tokenExpirationSeconds + extExpiresIn, refreshOnSeconds = refreshIn && refreshIn > 0 ? reqTimestamp + refreshIn : void 0;
      cachedAccessToken = function(homeAccountId, environment, accessToken, clientId, tenantId, scopes, expiresOn, extExpiresOn, base64Decode, correlationId, refreshOn, tokenType, userAssertionHash, keyId, additionalCacheKeyComponents) {
        const atEntity = {
          homeAccountId: homeAccountId,
        ...
        userAssertionHash && (atEntity.userAssertionHash = userAssertionHash), refreshOn && (atEntity.refreshOn = refreshOn.toString());
          ...
          normalizedTokenType) {
           case AuthenticationScheme.POP:
            ...
            if (!tokenClaims?.cnf?.kid) {
              throw createClientAuthError("token_claims_cnf_required_for_signedjwt", correlationId);
            }
           ...
           case "dpop":
            if (!keyId) {
              throw createClientAuthError("key_id_missing", correlationId);
            }
        ...
        return additionalCacheKeyComponents && Object.keys(additionalCacheKeyComponents).length > 0 && (atEntity.additionalCacheKeyComponents = additionalCacheKeyComponents), 
        atEntity;
  ...
  static async generateAuthenticationResult(cryptoObj, authority, cacheRecord, fromTokenCache, request, performanceClient, options = {}) {
    const {idTokenClaims: idTokenClaims, requestState: requestState, serverTokenResponse: serverTokenResponse, requestId: requestId, tokenBindingKeyManager: tokenBindingKeyManager = DEFAULT_TOKEN_BINDING_KEY_MANAGER} = options;
    let extExpiresOn, refreshOn, dpopProof, accessToken = "", responseScopes = [], expiresOn = null, familyId = "";
    if (cacheRecord.accessToken) {
      ...
      if (cacheRecord.accessToken.tokenType !== AuthenticationScheme.POP || request.popKid) {
        ...
        const popTokenGenerator = new PopTokenGenerator(cryptoObj, tokenBindingKeyManager, performanceClient), {secret: secret, keyId: keyId} = cacheRecord.accessToken;
        if (!keyId) {
          throw createClientAuthError("key_id_missing", request.correlationId);
        }
    ...
    const uid = idTokenClaims?.oid || idTokenClaims?.sub || "", tid = idTokenClaims?.tid || "", regionSubScope = idTokenClaims?.tenant_region_sub_scope;
    if ("string" == typeof regionSubScope && performanceClient?.addFields({
      regionSubScope: regionSubScope
    }, request.correlationId), serverTokenResponse?.spa_accountid && cacheRecord.account) {
      ...
      if (cacheRecord.account.tenantProfiles) {
        ...
        matchingProfile && (matchingProfile.nativeAccountId = serverTokenResponse.spa_accountid);
      }
    }
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ async function getClientAssertion(clientAssertion, clientId, tokenEndpoint, fmiPath) {
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ function getRequestThumbprint(clientId, request, homeAccountId) {
  return {
    clientId: clientId,
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ class ThrottlingUtils {
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ class NetworkError extends AuthError {
  constructor(error, httpStatus, responseHeaders) {
    super(error.errorCode, error.correlationId, error.errorMessage, error.subError), 
    Object.setPrototypeOf(this, NetworkError.prototype), this.name = "NetworkError", 
    this.error = error, this.httpStatus = httpStatus, this.responseHeaders = responseHeaders;
  }
}
...
async function addDpopTokenProofHeader(headers, request, tokenEndpoint, cryptoUtils, tokenBindingKeyManager) {
  ...
  const dpopProofGenerator = new DpopProofGenerator(cryptoUtils, tokenBindingKeyManager), keyId = request.dpopJkt;
  if (!keyId?.trim()) {
    throw createClientAuthError("key_id_missing", request.correlationId);
  }
  headers[HeaderNames_DPOP] = await dpopProofGenerator.generateTokenProof({
    tokenEndpoint: tokenEndpoint
...
async function sendPostRequest(thumbprint, tokenEndpoint, options, correlationId, cacheManager, networkClient, logger, performanceClient) {
  ...
  try {
    ...
    throw e instanceof AuthError ? e : createClientAuthError("network_error", correlationId);
  }
  return ThrottlingUtils.postProcess(cacheManager, thumbprint, response, correlationId), 
  response;
}

/*! @azure/msal-common v16.14.1 2026-09-15 */ class AuthorizationCodeClient {
  constructor(configuration, performanceClient) {
    this.includeRedirectUri = !0, this.config = buildClientConfiguration(configuration), 
    this.logger = new Logger(this.config.loggerOptions, name$1, "16.14.1"), this.cryptoUtils = this.config.cryptoInterface, 
  ...
  async acquireToken(request, apiId, authCodePayload) {
    if (!request.code) {
      throw createClientAuthError("request_cannot_be_made", request.correlationId);
    }
  ...
  async createTokenRequestBody(request) {
    ...
    this.includeRedirectUri) {
      ...
      throw createClientConfigurationError("redirect_uri_empty", request.correlationId);
    }
    if (addScopes(parameters, request.scopes, request.correlationId, !0, this.oidcDefaultScopes), 
    addResource(parameters, request.resource), request.attributeTokens && addAttributeTokens(parameters, request.attributeTokens), 
    this.performanceClient?.addFields({
      hasAttributeTokens: !!request.attributeTokens?.length
    }, request.correlationId), function(parameters, code) {
    ...
    request.authenticationScheme === AuthenticationScheme.POP) {
      ...
      if (!request.sshJwk) {
        throw createClientConfigurationError("missing_ssh_jwk", request.correlationId);
      }
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ function getStandardAuthorizeRequestParameters(authOptions, request, logger, performanceClient) {
  ...
  if (addScopes(parameters, [ ...request.scopes || [], ...request.extraScopesToConsent || [] ], request.correlationId, !0, authOptions.authority.options.OIDCOptions?.defaultScopes), 
  ...
  addCorrelationId(parameters, correlationId), function(parameters, responseMode) {
    ...
    if (request.sid && request.prompt === PromptValue_NONE) {
      ...
      /*! @azure/msal-common v16.14.1 2026-09-15 */ (request.account);
...
class RefreshTokenClient {
  constructor(configuration, performanceClient) {
    this.config = buildClientConfiguration(configuration), this.logger = new Logger(this.config.loggerOptions, name$1, "16.14.1"), 
  ...
  async acquireTokenByRefreshToken(request, apiId) {
    ...
    if (!request.account) {
      throw createClientAuthError("no_account_in_silent_request", request.correlationId);
    }
  ...
  async acquireTokenWithCachedRefreshToken(request, foci, apiId) {
    ...
    if (!refreshToken) {
      throw createInteractionRequiredAuthError("no_tokens_found", request.correlationId);
    }
    if (refreshToken.expiresOn) {
      ...
      if (this.performanceClient?.addFields({
        ...
        throw createInteractionRequiredAuthError("refresh_token_expired", request.correlationId);
      }
    }
  ...
  async executeTokenRequest(request, authority) {
    const queryParametersString = createTokenQueryParameters(request, this.config.authOptions.clientId, this.config.authOptions.redirectUri, this.performanceClient), endpoint = UrlString.appendQueryString(authority.tokenEndpoint, queryParametersString), requestBody = await invokeAsync(this.createTokenRequestBody.bind(this), "refreshTokenClientCreateTokenRequestBody", this.logger, this.performanceClient, request.correlationId)(request), headers = createTokenRequestHeaders(this.logger, this.config.systemOptions.preventCorsPreflight, request.ccsCredential);
    await addDpopTokenProofHeader(headers, request, endpoint, this.cryptoUtils, this.config.tokenBindingKeyManager);
    const thumbprint = getRequestThumbprint(this.config.authOptions.clientId, request);
  ...
  async createTokenRequestBody(request) {
    const parameters = new Map;
    if (addClientId(parameters, request.embeddedClientId || request.extraParameters?.client_id || this.config.authOptions.clientId), 
    request.redirectUri && addRedirectUri(parameters, request.redirectUri), addScopes(parameters, request.scopes, request.correlationId, !0, this.config.authOptions.authority.options.OIDCOptions?.defaultScopes), 
    ...
    }(parameters, request.refreshToken), request.attributeTokens && addAttributeTokens(parameters, request.attributeTokens), 
    this.performanceClient?.addFields({
      hasAttributeTokens: !!request.attributeTokens?.length
    }, request.correlationId), this.config.clientCredentials.clientSecret && addClientSecret(parameters, this.config.clientCredentials.clientSecret), 
    this.config.clientCredentials.clientAssertion) {
      const clientAssertion = this.config.clientCredentials.clientAssertion;
      addClientAssertion(parameters, await getClientAssertion(clientAssertion.assertion, this.config.authOptions.clientId, request.resourceRequestUri)), 
      addClientAssertionType(parameters, clientAssertion.assertionType);
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ function makeExtraSkuString(params) {
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ class SilentFlowClient {
  constructor(configuration, performanceClient) {
    this.config = buildClientConfiguration(configuration), this.logger = new Logger(this.config.loggerOptions, name$1, "16.14.1"), 
    this.cryptoUtils = this.config.cryptoInterface, this.cacheManager = this.config.storageInterface, 
    this.networkClient = this.config.networkInterface, this.serverTelemetryManager = this.config.serverTelemetryManager, 
  ...
  async acquireCachedToken(request) {
    ...
    if (request.forceRefresh || !StringUtils.isEmptyObj(request.claims)) {
      ...
      createClientAuthError("token_refresh_required", request.correlationId);
    }
    if (!request.account) {
      throw createClientAuthError("no_account_in_silent_request", request.correlationId);
    }
    const requestTenantId = request.account.tenantId || function(authority, correlationId) {
      const authorityUrlComponents = new UrlString(authority, correlationId).getUrlComponents(), tenantId = authorityUrlComponents.PathSegments.slice(-1)[0]?.toLowerCase();
      switch (tenantId) {
    ...
    }(request.authority, request.correlationId), tokenKeys = this.cacheManager.getTokenKeys(), cachedAccessToken = this.cacheManager.getAccessToken(request.account, request, tokenKeys, requestTenantId);
    ...
    if (cachedAccessTokenType === AuthenticationScheme.DPOP.toLowerCase()) {
      if (!cachedAccessToken.keyId) {
        throw this.logger.info("SilentFlowClient:acquireCachedToken - Cached DPoP access token is missing keyId; refresh required", request.correlationId), 
        this.setCacheOutcome(CacheOutcome_NO_CACHED_ACCESS_TOKEN, request.correlationId), 
        createClientAuthError("token_refresh_required", request.correlationId);
      }
      try {
        await this.config.tokenBindingKeyManager.getTokenBindingPublicKeyJwk(cachedAccessToken.keyId, request.correlationId);
      } catch {
        throw this.logger.info("SilentFlowClient:acquireCachedToken - Local DPoP key was not found for cached access token; refresh required", request.correlationId), 
        this.setCacheOutcome(CacheOutcome_NO_CACHED_ACCESS_TOKEN, request.correlationId), 
        createClientAuthError("token_refresh_required", request.correlationId);
      }
    }
  ...
  async generateResultFromCacheRecord(cacheRecord, request) {
    let idTokenClaims;
    return cacheRecord.idToken && (idTokenClaims = extractTokenClaims(cacheRecord.idToken.secret, this.config.cryptoInterface.base64Decode, request.correlationId)), 
    ResponseHandler.generateAuthenticationResult(this.cryptoUtils, this.authority, cacheRecord, !0, request, this.performanceClient, {
      idTokenClaims: idTokenClaims,
...
/*! @azure/msal-common v16.14.1 2026-09-15 */ function enforceResourceParameter(isMcp, request) {
  if (isMcp) {
    if (request.resource && (containsResourceParam(request.extraParameters) || containsResourceParam(request.extraQueryParameters))) {
      throw createClientAuthError("misplaced_resource_parameter", request.correlationId || "");
    }
    if (!request.resource) {
      throw createClientAuthError("resource_parameter_required", request.correlationId || "");
    }
  }
}
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ class Deserializer {
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ const NodeAuthErrorMessage_invalidLoopbackAddressType = {
...
class NodeAuthError extends AuthError {
  constructor(errorCode, correlationId, errorMessage) {
    super(errorCode, correlationId, errorMessage), this.name = "NodeAuthError";
  }
  ...
  static createNoAuthCodeInResponseError(correlationId = "") {
    return new NodeAuthError(NodeAuthErrorMessage_noAuthCodeInResponse.code, correlationId, `${NodeAuthErrorMessage_noAuthCodeInResponse.desc}`);
  }
  ...
  static createLoopbackServerTimeoutError(correlationId = "") {
    return new NodeAuthError(NodeAuthErrorMessage_loopbackServerTimeout.code, correlationId, `${NodeAuthErrorMessage_loopbackServerTimeout.desc}`);
  }
  static createStateNotFoundError(correlationId = "") {
    return new NodeAuthError(NodeAuthErrorMessage_stateNotFoundError.code, correlationId, NodeAuthErrorMessage_stateNotFoundError.desc);
  }
  ...
  static createRedirectUriNotSupportedError(correlationId = "") {
    return new NodeAuthError(NodeAuthErrorMessage_redirectUriNotSupported.code, correlationId, NodeAuthErrorMessage_redirectUriNotSupported.desc);
  }
}

/*! @azure/msal-node v6.0.1 2026-09-15 */ const DEFAULT_AUTH_OPTIONS = {
  clientId: "",
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ class EncodingUtils {
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ class HashUtils {
  sha256(buffer) {
    return crypto$1.createHash(Hash_SHA256).update(buffer).digest();
  }
}

/*! @azure/msal-node v6.0.1 2026-09-15 */ class PkceGenerator {
  constructor() {
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ class CryptoProvider {
  constructor() {
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ function generateCredentialKey(credential, hash) {
  const familyId = credential.credentialType === CredentialType.REFRESH_TOKEN && credential.familyId || credential.clientId, scheme = credential.tokenType && credential.tokenType.toLowerCase() !== AuthenticationScheme.BEARER.toLowerCase() ? credential.tokenType.toLowerCase() : "", credentialKey = [ credential.homeAccountId, credential.environment, credential.credentialType, familyId, credential.realm || "", credential.target || "", scheme ];
  return credential.additionalCacheKeyComponents && Object.keys(credential.additionalCacheKeyComponents).length > 0 && credentialKey.push(hash ?? function(components) {
  ...
  }(credential.additionalCacheKeyComponents)), credentialKey.join(CACHE_KEY_SEPARATOR$0).toLowerCase();
}
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ const defaultSerializedCache_Account = {}, defaultSerializedCache_IdToken = {}, defaultSerializedCache_AccessToken = {}, defaultSerializedCache_RefreshToken = {}, defaultSerializedCache_AppMetadata = {};
       ...
       case "pre":
        {
          ...
          if (identifier) {
            ...
            if (!1 === identifierBase && (prerelease = [ identifier ]), ((prerelease, identifier) => {
              const identifiers = identifier.split(".");
              if (identifiers.length > prerelease.length) {
                return !1;
              }
              for (let i = 0; i < identifiers.length; i++) {
                if (0 !== compareIdentifiers(prerelease[i], identifiers[i])) {
                  return !1;
                }
              }
              return !0;
...
function requireRange() {
  ...
  })), parseOptions = requireParseOptions(), Comparator = requireComparator(), debug = requireDebug(), SemVer = requireSemver$1(), {safeRe: re, src: src, t: t, comparatorTrimReplace: comparatorTrimReplace, tildeTrimReplace: tildeTrimReplace, caretTrimReplace: caretTrimReplace} = requireRe(), {FLAG_INCLUDE_PRERELEASE: FLAG_INCLUDE_PRERELEASE, FLAG_LOOSE: FLAG_LOOSE} = requireConstants(), BUILDSTRIPRE = new RegExp(src[t.BUILD], "g"), isNullSet = c => "<0.0.0-0" === c.value, isAny = c => "" === c.value, isSatisfiable = (comparators, options) => {
    let result = !0;
  ...
  comp = replaceStars(comp, options), debug("stars", comp), comp), isX = id => !id || "x" === id.toLowerCase() || "*" === id, replaceTildes = (comp, options) => comp.trim().split(/\s+/).map(c => replaceTilde(c, options)).join(" "), replaceTilde = (comp, options) => {
    const r = options.loose ? re[t.TILDELOOSE] : re[t.TILDE], z = options.includePrerelease ? "-0" : "";
    return comp.replace(r, (_, M, m, p, pr) => {
      let ret;
      return debug("tilde", comp, _, M, m, p, pr), isX(M) ? ret = "" : isX(m) ? ret = `>=${M}.0.0${z} <${+M + 1}.0.0-0` : isX(p) ? ret = `>=${M}.${m}.0${z} <${M}.${+m + 1}.0-0` : pr ? (debug("replaceTilde pr", pr), 
    ...
    return comp.replace(r, (_, M, m, p, pr) => {
      ...
      ret = "0" === M ? "0" === m ? `>=${M}.${m}.${p} <${M}.${m}.${+p + 1}-0` : `>=${M}.${m}.${p} <${M}.${+m + 1}.0-0` : `>=${M}.${m}.${p} <${+M + 1}.0.0-0`), 
  ...
  comp.split(/\s+/).map(c => replaceXRange(c, options)).join(" ")), replaceXRange = (comp, options) => {
    ...
    return comp.replace(r, (ret, gtlt, M, m, p, pr) => {
      if (debug("xRange", comp, ret, gtlt, M, m, p, pr), ((M, m, p) => isX(M) && !isX(m) || isX(m) && p && !isX(p))(M, m, p)) {
        return comp;
      }
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ const name = "@azure/msal-node";

/*! @azure/msal-node v6.0.1 2026-09-15 */
class BaseClient {
  constructor(configuration) {
    this.config = buildClientConfiguration(configuration), this.logger = new Logger(this.config.loggerOptions, name, "6.0.1"), 
    this.cryptoUtils = this.config.cryptoInterface, this.cacheManager = this.config.storageInterface, 
    this.networkClient = this.config.networkInterface, this.serverTelemetryManager = this.config.serverTelemetryManager, 
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ class UsernamePasswordClient extends BaseClient {
  constructor(configuration) {
    super(configuration);
  }
  async acquireToken(request) {
    this.logger.info("in acquireToken call in username-password client", request.correlationId);
    const reqTimestamp = nowSeconds(), response = await this.executeTokenRequest(this.authority, request), responseHandler = new ResponseHandler(this.config.authOptions.clientId, this.cacheManager, this.cryptoUtils, this.logger, this.performanceClient, this.config.serializableCache, this.config.persistencePlugin, this.config.tokenBindingKeyManager);
  ...
  async createTokenRequestBody(request) {
    ...
    }(parameters, request.password), addScopes(parameters, request.scopes, request.correlationId), 
    addResponseType(parameters, OAuthResponseType_IDTOKEN_TOKEN), addGrantType(parameters, GrantType_RESOURCE_OWNER_PASSWORD_GRANT), 
    ...
    addClientAssertionType(parameters, clientAssertion.assertionType)), (!StringUtils.isEmptyObj(request.claims) || this.config.authOptions.clientCapabilities && this.config.authOptions.clientCapabilities.length > 0) && addClaims(parameters, request.correlationId, request.claims, this.config.authOptions.clientCapabilities), 
    this.config.systemOptions.preventCorsPreflight && request.username && addCcsUpn(parameters, request.username), 
    mapToQueryString(parameters);
  }
}

/*! @azure/msal-node v6.0.1 2026-09-15 */ function getAuthCodeRequestUrl(config, authority, request, logger) {
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ class ClientApplication {
  constructor(configuration) {
    ...
    }(configuration), this.cryptoProvider = new CryptoProvider, this.logger = new Logger(this.config.system.loggerOptions, name, "6.0.1"), 
  ...
  async acquireTokenByCode(request, authCodePayLoad) {
    ...
    request.state && validatedAuthCodePayload && (this.logger.info("acquireTokenByCode - validating state", request.correlationId || ""), 
    this.validateState(request.state, validatedAuthCodePayload.state || "", request.correlationId || ""), 
    validatedAuthCodePayload = {
      ...validatedAuthCodePayload,
      state: ""
    });
    const expectedNonce = void 0 !== request.nonce ? request.nonce : validatedAuthCodePayload?.nonce;
    validatedAuthCodePayload = {
      ...validatedAuthCodePayload,
      code: request.code,
      nonce: expectedNonce
    ...
    try {
      ...
      await authorizationCodeClient.acquireToken(validRequest, ApiId_acquireTokenByCode, validatedAuthCodePayload);
    } catch (e) {
      throw e instanceof AuthError && (e.correlationId = validRequest.correlationId), 
  ...
  async acquireTokenSilent(request) {
    ...
    try {
      ...
      throw error instanceof AuthError && (error.correlationId = validRequest.correlationId), 
  ...
  validateState(state, cachedState, correlationId) {
    ...
    if (state !== cachedState) {
      throw createClientAuthError("state_mismatch", correlationId);
    }
  }
  getLogger() {
    return this.logger;
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ class LoopbackClient {
  ...
  async listenForAuthCode(successTemplate, errorTemplate) {
    ...
    return new Promise((resolve, reject) => {
      this.server = http.createServer((req, res) => {
        ...
        if ("GET" !== method && "POST" !== method) {
          return res.writeHead(405, {
            Allow: "GET, POST"
          }), void res.end("Method Not Allowed");
        }
        const url = req.url;
        if (!url) {
        ...
        if ("/" === url) {
          return "POST" === method ? void this.handlePostRequest(req, res, resolve, successTemplate, errorTemplate) : void res.end(successTemplate || "Auth code was successfully acquired. You can close this window now.");
        }
        if ("GET" === method) {
          ...
          if (!authCodeResponse.code && !authCodeResponse.error) {
            return res.writeHead(200), void res.end();
          }
      ...
      this.server.on("error", err => {
        "EADDRINUSE" === err.code && this.preferredPort && 0 !== port ? this.server?.listen(0, "127.0.0.1") : reject(err);
  ...
  handlePostRequest(req, res, resolve, successTemplate, errorTemplate) {
    ...
    if ("application/x-www-form-urlencoded" !== contentType) {
      return res.writeHead(415), void res.end("Unsupported Media Type");
    }
    let body = "";
    req.on("error", () => {
      res.headersSent || res.writeHead(400), res.end();
    }), req.on("data", chunk => {
      body += chunk.toString();
    }), req.on("end", () => {
      ...
      authCodeResponse.error ? (res.writeHead(200), res.end(errorTemplate || `Error occurred: ${authCodeResponse.error}`)) : (res.writeHead(200), 
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ class DeviceCodeClient extends BaseClient {
  constructor(configuration) {
    super(configuration);
  }
  async acquireToken(request) {
  ...
  createQueryString(request) {
    const parameters = new Map;
    return addScopes(parameters, request.scopes, request.correlationId), addClientId(parameters, this.config.authOptions.clientId), 
    request.extraQueryParameters && addExtraParameters(parameters, request.extraQueryParameters), 
    (request.claims || this.config.authOptions.clientCapabilities && this.config.authOptions.clientCapabilities.length > 0) && addClaims(parameters, request.correlationId, request.claims, this.config.authOptions.clientCapabilities), 
    mapToQueryString(parameters);
  }
  continuePolling(deviceCodeExpirationTime, correlationId, userSpecifiedTimeout, userSpecifiedCancelFlag) {
    if (userSpecifiedCancelFlag) {
      throw this.logger.error("Token request cancelled by setting DeviceCodeRequest.cancel = true", correlationId), 
      createClientAuthError("device_code_polling_cancelled", correlationId);
    }
    if (userSpecifiedTimeout && userSpecifiedTimeout < deviceCodeExpirationTime && nowSeconds() > userSpecifiedTimeout) {
      throw this.logger.error(`User defined timeout for device code polling reached. The timeout was set for ${userSpecifiedTimeout}`, correlationId), 
      createClientAuthError("user_timeout_reached", correlationId);
    }
    if (nowSeconds() > deviceCodeExpirationTime) {
      throw userSpecifiedTimeout && this.logger.verbose(`User specified timeout ignored as the device code has expired before the timeout elapsed. The user specified timeout was set for ${userSpecifiedTimeout}`, correlationId), 
      this.logger.error(`Device code expired. Expiration time of device code was ${deviceCodeExpirationTime}`, correlationId), 
      createClientAuthError("device_code_expired", correlationId);
    }
    return !0;
  }
  async acquireTokenWithDeviceCode(request, deviceCodeResponse) {
    ...
    for (;this.continuePolling(deviceCodeExpirationTime, request.correlationId, userSpecifiedTimeout, request.cancel); ) {
      const thumbprint = {
        clientId: this.config.authOptions.clientId,
        authority: request.authority,
        scopes: request.scopes,
      ...
      if ("authorization_pending" !== response.body.error) {
        ...
        createAuthError("post_request_failed", request.correlationId, response.body.error);
      }
    ...
    createClientAuthError("device_code_unknown_error", request.correlationId);
  }
  createTokenRequestBody(request, deviceCodeResponse) {
    const parameters = new Map;
    addScopes(parameters, request.scopes, request.correlationId), addClientId(parameters, this.config.authOptions.clientId), 
    ...
    addApplicationTelemetry(parameters, this.config.telemetry.application), addThrottling(parameters), 
    this.serverTelemetryManager && addServerTelemetry(parameters, this.serverTelemetryManager), 
    (!StringUtils.isEmptyObj(request.claims) || this.config.authOptions.clientCapabilities && this.config.authOptions.clientCapabilities.length > 0) && addClaims(parameters, request.correlationId, request.claims, this.config.authOptions.clientCapabilities), 
    mapToQueryString(parameters);
  }
}

/*! @azure/msal-node v6.0.1 2026-09-15 */ class PublicClientApplication extends ClientApplication {
  constructor(configuration) {
  ...
  async acquireTokenInteractive(request) {
    ...
    const {openBrowser: openBrowser, successTemplate: successTemplate, errorTemplate: errorTemplate, windowHandle: windowHandle, preferredPort: preferredPort, ...remainingProperties} = request;
    ...
    const {verifier: verifier, challenge: challenge} = await this.cryptoProvider.generatePkceCodes(), loopbackClient = new LoopbackClient(preferredPort), responseMode = remainingProperties.responseMode ?? ResponseMode_FORM_POST;
    if (responseMode !== ResponseMode_QUERY && responseMode !== ResponseMode_FORM_POST) {
      throw createClientConfigurationError("invalid_response_mode", correlationId);
    }
    ...
    try {
      ...
      }), redirectUri = await this.waitForRedirectUri(loopbackClient, correlationId), validRequest = {
        ...remainingProperties,
        correlationId: correlationId,
        scopes: request.scopes || OIDC_DEFAULT_SCOPES,
        redirectUri: redirectUri,
    ...
    return new Promise((resolve, reject) => {
      ...
      const id = setInterval(() => {
        if (LOOPBACK_SERVER_CONSTANTS_TIMEOUT_MS / LOOPBACK_SERVER_CONSTANTS_INTERVAL_MS < ticks) {
          return clearInterval(id), void reject(NodeAuthError.createLoopbackServerTimeoutError(correlationId));
        }
        try {
...
/*! @azure/msal-node v6.0.1 2026-09-15 */ process.env.ProgramData, process.env.ProgramFiles;
...
class AuthProvider {
  ...
  constructor(msalConfig) {
    this.clientApplication = new PublicClientApplication(msalConfig), this.account = void 0;
  }
  logDebug(...args) {
  ...
  async logout() {
    ...
    try {
      await cache.removeAccount(this.account), this.account = void 0;
    } catch (error) {
      this.account = void 0, this.logError("Failed to clear token cache.", error);
    }
  }
  async getToken(request, deviceCodeCallback) {
...
class OneDrivePhotos extends node_events.EventEmitter {
    ...
    for (;attempt < maxRetries; ) {
      ...
      try {
        ...
        if (!tokenResponse?.accessToken) {
          throw this.logError("No access token returned from AuthProvider.getToken"), new Error("No access token returned from AuthProvider.getToken");
        }
        this.log("onAuthReady token responded"), this.#graphClient = Client.init({
          authProvider: done => {
            done(null, tokenResponse.accessToken);
          }
        });
        const graphResponse = await this.#graphClient.api(protectedResources.graphMe.endpoint).get(), userId = graphResponse?.id;
        if (!userId) {
          this.#graphClient = void 0;
          const err = new Error("Microsoft Graph /me response is missing required user id");
          throw this.logError(err.message), err;
        }
        return this.#userId = userId, this.log(`onAuthReady done, retry count: ${attempt}`), 
        ...
        if (![ "UnknownError", "TypeError", "InvalidAuthenticationToken" ].includes(errorCode)) {
  ...
  async request(logContext, url, method = "get", data = null) {
    if (this.logDebug((logContext ? `[${logContext}]` : "") + ` request ${method} URL: ${url}`), 
    !this.#graphClient) {
      ...
      throw this.logError(message), new Error(message);
    }
    try {
