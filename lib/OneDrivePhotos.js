...
var RequestMethod, Log = require("logger"), node_stream = require("node:stream");
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class Serializer {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ const Constants$1 = {
...
class AuthError extends Error {
  constructor(errorCode, errorMessage, suberror) {
    ...
    this.errorCode = errorCode || Constants$1.EMPTY_STRING, this.errorMessage = errorMessage || Constants$1.EMPTY_STRING, 
    this.subError = suberror || Constants$1.EMPTY_STRING, this.name = "AuthError";
  }
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ const DEFAULT_CRYPTO_IMPLEMENTATION = {
  createNewGuid: () => {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ var LogLevel;
...
class Logger {
  constructor(loggerOptions, packageName, packageVersion) {
    ...
    this.packageName = packageName || Constants$1.EMPTY_STRING, this.packageVersion = packageVersion || Constants$1.EMPTY_STRING;
  }
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class StringUtils {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class ScopeSet {
  ...
  static fromString(inputScopeString) {
    const inputScopes = (inputScopeString || Constants$1.EMPTY_STRING).split(" ");
  ...
  static createSearchScopes(inputScopeString) {
    ...
    return scopeSet.containsOnlyOIDCScopes() ? scopeSet.removeScope(Constants$1.OFFLINE_ACCESS_SCOPE) : scopeSet.removeOIDCScopes(), 
    scopeSet;
  }
  containsScope(scope) {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ function buildClientInfo(rawClientInfo, base64Decode) {
  if (!rawClientInfo) {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ function tenantIdMatchesHomeTenant(tenantId, homeAccountId) {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ const AuthorityType_Default = 0, AuthorityType_Adfs = 1, AuthorityType_Dsts = 2, AuthorityType_Ciam = 3;

/*! @azure/msal-common v15.13.3 2025-12-04 */ function getTenantIdFromIdTokenClaims(idTokenClaims) {
  if (idTokenClaims) {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ const ProtocolMode_AAD = "AAD", ProtocolMode_OIDC = "OIDC";

/*! @azure/msal-common v15.13.3 2025-12-04 */ class AccountEntity {
  static getAccountInfo(accountEntity) {
    return {
      ...
      tenantProfiles: new Map((accountEntity.tenantProfiles || []).map(tenantProfile => [ tenantProfile.tenantId, tenantProfile ])),
  ...
  static createAccount(accountDetails, authority, base64Decode) {
    ...
    let clientInfo;
    authority.authorityType === AuthorityType_Adfs ? account.authorityType = CacheAccountType_ADFS_ACCOUNT_TYPE : authority.protocolMode === ProtocolMode_OIDC ? account.authorityType = CacheAccountType_GENERIC_ACCOUNT_TYPE : account.authorityType = CacheAccountType_MSSTS_ACCOUNT_TYPE, 
    accountDetails.clientInfo && base64Decode && (clientInfo = buildClientInfo(accountDetails.clientInfo, base64Decode), 
    clientInfo.xms_tdbr && (account.dataBoundary = "EU" === clientInfo.xms_tdbr ? "EU" : "None")), 
    ...
    if (account.username = preferredUsername || email || "", account.loginHint = accountDetails.idTokenClaims?.login_hint, 
    account.name = accountDetails.idTokenClaims?.name || "", account.cloudGraphHostName = accountDetails.cloudGraphHostName, 
    account.msGraphHost = accountDetails.msGraphHost, accountDetails.tenantProfiles) {
  ...
  static createFromAccountInfo(accountInfo, cloudGraphHostName, msGraphHost) {
    ...
    account.name = accountInfo.name, account.loginHint = accountInfo.loginHint, account.cloudGraphHostName = cloudGraphHostName, 
    account.msGraphHost = msGraphHost, account.tenantProfiles = Array.from(accountInfo.tenantProfiles?.values() || []), 
    account.dataBoundary = accountInfo.dataBoundary, account;
  }
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ function extractTokenClaims(encodedToken, base64Decode) {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ function getDeserializedResponse(responseString) {
...
function mapToQueryString(parameters, encodeExtraParams = !0, extraQueryParameters) {
  const queryParameterArray = new Array;
  return parameters.forEach((value, key) => {
    !encodeExtraParams && extraQueryParameters && key in extraQueryParameters ? queryParameterArray.push(`${key}=${value}`) : queryParameterArray.push(`${key}=${encodeURIComponent(value)}`);
  }), queryParameterArray.join("&");
}

/*! @azure/msal-common v15.13.3 2025-12-04 */ class UrlString {
  get urlString() {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ const EndpointMetadata = {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ InstanceDiscoveryMetadata.metadata.forEach(metadataEntry => {
  metadataEntry.aliases.forEach(alias => {
    InstanceDiscoveryMetadataAliases.add(alias);
  });
});

const cacheQuotaExceeded = "cache_quota_exceeded", cacheErrorUnknown = "cache_error_unknown", CacheErrorMessages = {
...
class CacheManager {
  constructor(clientId, cryptoImpl, logger, performanceClient, staticAuthorityOptions) {
    this.clientId = clientId, this.cryptoImpl = cryptoImpl, this.commonLogger = logger.clone("@azure/msal-common", "15.13.3"), 
  ...
  getAccountInfoFilteredBy(accountFilter, correlationId) {
    if (0 === Object.keys(accountFilter).length || Object.values(accountFilter).every(value => !value)) {
      return this.commonLogger.warning("getAccountInfoFilteredBy: Account filter is empty or invalid, returning null"), 
      null;
    }
  ...
  getBaseAccountInfo(accountFilter, correlationId) {
    ...
    return accountEntities.length > 0 ? AccountEntity.getAccountInfo(accountEntities[0]) : null;
  }
  ...
  async saveCacheRecord(cacheRecord, correlationId, kmsi, storeInCache) {
    ...
    try {
      cacheRecord.account && await this.setAccount(cacheRecord.account, correlationId, kmsi), 
      ...
      throw this.commonLogger?.error("CacheManager.saveCacheRecord: failed"), e instanceof AuthError ? e : function(e) {
  ...
  removeAccessToken(key, correlationId) {
    ...
    if (this.removeItem(key, correlationId), this.performanceClient.incrementFields({
      accessTokensRemoved: 1
    }, correlationId), !credential || credential.credentialType.toLowerCase() !== CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME.toLowerCase() || credential.tokenType !== AuthenticationScheme.POP) {
      return;
    }
    const kid = credential.keyId;
    kid && this.cryptoImpl.removeTokenBindingKey(kid).catch(() => {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ const DEFAULT_SYSTEM_OPTIONS$1 = {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ const CcsCredentialType_HOME_ACCOUNT_ID = "home_account_id", CcsCredentialType_UPN = "UPN", RETURN_SPA_CODE = "return_spa_code", X_CLIENT_EXTRA_SKU = "x-client-xtra-sku";
...
function addThrottling(parameters) {
  parameters.set("x-ms-lib-capability", ThrottlingConstants_X_MS_LIB_CAPABILITY_VALUE);
}
...
class RegionDiscovery {
  ...
  async detectRegion(environmentRegion, regionDiscoveryMetadata) {
    ...
    if (autodetectedRegionName) {
      ...
      try {
        const localIMDSVersionResponse = await invokeAsync(this.getRegionFromIMDS.bind(this), PerformanceEvents_RegionDiscoveryGetRegionFromIMDS, this.logger, this.performanceClient, this.correlationId)(Constants$1.IMDS_VERSION, options);
        if (localIMDSVersionResponse.status === HttpStatus_SUCCESS && (autodetectedRegionName = localIMDSVersionResponse.body, 
        regionDiscoveryMetadata.region_source = RegionDiscoverySources_IMDS), localIMDSVersionResponse.status === HttpStatus_BAD_REQUEST) {
          ...
          currentIMDSVersionResponse.status === HttpStatus_SUCCESS && (autodetectedRegionName = currentIMDSVersionResponse.body, 
  ...
  async getRegionFromIMDS(version, options) {
    ...
    this.networkInterface.sendGetRequestAsync(`${Constants$1.IMDS_ENDPOINT}?api-version=${version}&format=text`, options, Constants$1.IMDS_TIMEOUT);
  }
  async getCurrentVersion(options) {
    ...
    try {
      ...
      return response.status === HttpStatus_BAD_REQUEST && response.body && response.body["newest-versions"] && response.body["newest-versions"].length > 0 ? response.body["newest-versions"][0] : null;
    } catch (e) {
      return null;
    }
  }
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ function isCredentialEntity(entity) {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ RegionDiscovery.IMDS_OPTIONS = {
  headers: {
    Metadata: "true"
  }
};
...
class Authority {
  ...
  async getEndpointMetadataFromNetwork() {
    ...
    try {
      ...
      /*! @azure/msal-common v15.13.3 2025-12-04 */ (response.body);
  ...
  async getCloudDiscoveryMetadataFromNetwork() {
    ...
    const instanceDiscoveryEndpoint = `${Constants$1.AAD_INSTANCE_DISCOVERY_ENDPT}${this.canonicalAuthority}oauth2/v2.0/authorize`, options = {};
    let match = null;
    try {
      ...
      /*! @azure/msal-common v15.13.3 2025-12-04 */ (response.body)) {
        ...
        typedResponseBody = response.body, typedResponseBody.error === Constants$1.INVALID_INSTANCE) {
  ...
  static buildRegionalAuthorityString(host, region, queryString) {
    ...
    this.isPublicCloudAuthority(authorityUrlParts.HostNameAndPort) && (hostNameAndPort = `${region}.${Constants$1.REGIONAL_AUTH_PUBLIC_CLOUD_SUFFIX}`);
  ...
  static transformCIAMAuthority(authority) {
    ...
    if (0 === authorityUrlComponents.PathSegments.length && authorityUrlComponents.HostNameAndPort.endsWith(Constants$1.CIAM_AUTH_URL)) {
      ciamAuthority = `${ciamAuthority}${authorityUrlComponents.HostNameAndPort.split(".")[0]}${Constants$1.AAD_TENANT_DOMAIN_SUFFIX}`;
    }
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ Authority.reservedTenantDomains = new Set([ "{tenant}", "{tenantid}", AADAuthorityConstants_COMMON, AADAuthorityConstants_CONSUMERS, AADAuthorityConstants_ORGANIZATIONS ]);
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ function getRequestThumbprint(clientId, request, homeAccountId) {
  return {
    clientId: clientId,
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class ThrottlingUtils {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class NetworkError extends AuthError {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class BaseClient {
  constructor(configuration, performanceClient) {
    this.config = function({authOptions: userAuthOptions, systemOptions: userSystemOptions, loggerOptions: userLoggerOption, cacheOptions: userCacheOptions, storageInterface: storageImplementation, networkInterface: networkImplementation, cryptoInterface: cryptoImplementation, clientCredentials: clientCredentials, libraryInfo: libraryInfo, telemetry: telemetry, serverTelemetryManager: serverTelemetryManager, persistencePlugin: persistencePlugin, serializableCache: serializableCache}) {
      ...
      var authOptions;
    }(configuration), this.logger = new Logger(this.config.loggerOptions, "@azure/msal-common", "15.13.3"), 
  ...
  createTokenRequestHeaders(ccsCred) {
    const headers = {};
    if (headers[HeaderNames_CONTENT_TYPE] = Constants$1.URL_FORM_CONTENT_TYPE, !this.config.systemOptions.preventCorsPreflight && ccsCred) {
      switch (ccsCred.type) {
  ...
  async sendPostRequest(thumbprint, tokenEndpoint, options, correlationId) {
    ...
    return ThrottlingUtils.postProcess(this.cacheManager, thumbprint, response, correlationId), 
    response;
  }
  ...
  createTokenQueryParameters(request) {
    ...
    request.tokenQueryParameters && addExtraQueryParameters(parameters, request.tokenQueryParameters), 
    addCorrelationId(parameters, request.correlationId), instrumentBrokerParams(parameters, request.correlationId, this.performanceClient), 
    mapToQueryString(parameters);
  }
}

/*! @azure/msal-common v15.13.3 2025-12-04 */ const noTokensFound = "no_tokens_found", nativeAccountUnavailable = "native_account_unavailable", refreshTokenExpired = "refresh_token_expired", uxNotAllowed = "ux_not_allowed", badToken = "bad_token", InteractionRequiredServerErrorMessage = [ "interaction_required", "consent_required", "login_required", "bad_token", "ux_not_allowed" ], InteractionRequiredAuthSubErrorMessage = [ "message_only", "additional_action", "basic_action", "user_password_expired", "consent_required", "bad_token" ], InteractionRequiredAuthErrorMessages = {
...
class InteractionRequiredAuthError extends AuthError {
  constructor(errorCode, errorMessage, subError, timestamp, traceId, correlationId, claims, errorNo) {
    ...
    this.timestamp = timestamp || Constants$1.EMPTY_STRING, this.traceId = traceId || Constants$1.EMPTY_STRING, 
    this.correlationId = correlationId || Constants$1.EMPTY_STRING, this.claims = claims || Constants$1.EMPTY_STRING, 
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class ProtocolUtils {
  ...
  static parseRequestState(cryptoObj, state) {
    ...
    try {
      const splitState = state.split(Constants$1.RESOURCE_DELIM), libraryState = splitState[0], userState = splitState.length > 1 ? splitState.slice(1).join(Constants$1.RESOURCE_DELIM) : Constants$1.EMPTY_STRING, libraryStateString = cryptoObj.base64Decode(libraryState), libraryStateObj = JSON.parse(libraryStateString);
      return {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ const KeyLocation_SW = "sw";
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class TokenCacheContext {
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class ResponseHandler {
  ...
  async handleServerTokenResponse(serverTokenResponse, authority, reqTimestamp, request, authCodePayload, userAssertionHash, handlingRefreshTokenResponse, forceCacheRefreshTokenResponse, serverRequestId) {
    ...
    serverTokenResponse.id_token) {
      if (idTokenClaims = extractTokenClaims(serverTokenResponse.id_token || Constants$1.EMPTY_STRING, this.cryptoObj.base64Decode), 
    ...
    try {
      ...
      handlingRefreshTokenResponse && !forceCacheRefreshTokenResponse && cacheRecord.account) {
        ...
        if (!this.cacheStorage.getAccount(key, request.correlationId)) {
      ...
      }(idTokenClaims || {}), request.storeInCache);
    } finally {
  ...
  generateCacheRecord(serverTokenResponse, authority, reqTimestamp, request, idTokenClaims, userAssertionHash, authCodePayload) {
    ...
    }, cachedAccount = function(cacheStorage, authority, homeAccountId, base64Decode, correlationId, idTokenClaims, clientInfo, environment, claimsTenantId, authCodePayload, nativeAccountId, logger) {
    ...
    /*! @azure/msal-common v15.13.3 2025-12-04 */ (this.cacheStorage, authority, this.homeAccountIdentifier, this.cryptoObj.base64Decode, request.correlationId, idTokenClaims, serverTokenResponse.client_info, env, claimsTenantId, authCodePayload, void 0, this.logger));
  ...
  static async generateAuthenticationResult(cryptoObj, authority, cacheRecord, fromTokenCache, request, idTokenClaims, requestState, serverTokenResponse, requestId) {
    let extExpiresOn, refreshOn, accessToken = Constants$1.EMPTY_STRING, responseScopes = [], expiresOn = null, familyId = Constants$1.EMPTY_STRING;
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class AuthorizationCodeClient extends BaseClient {
  constructor(configuration, performanceClient) {
  ...
  async executeTokenRequest(authority, request) {
    ...
    const headers = this.createTokenRequestHeaders(ccsCredential || request.ccsCredential), thumbprint = getRequestThumbprint(this.config.authOptions.clientId, request);
  ...
  async createTokenRequestBody(request) {
    ...
    const parameters = new Map;
    if (addClientId(parameters, request.embeddedClientId || request.tokenBodyParameters?.client_id || this.config.authOptions.clientId), 
    this.includeRedirectUri) {
      addRedirectUri(parameters, request.redirectUri);
    } else if (!request.redirectUri) {
      throw createClientConfigurationError("redirect_uri_empty");
    }
    if (addScopes(parameters, request.scopes, !0, this.oidcDefaultScopes), function(parameters, code) {
    ...
    return request.embeddedClientId && addBrokerParameters(parameters, this.config.authOptions.clientId, this.config.authOptions.redirectUri), 
    request.tokenBodyParameters && addExtraQueryParameters(parameters, request.tokenBodyParameters), 
    !request.enableSpaAuthorizationCode || request.tokenBodyParameters && request.tokenBodyParameters.return_spa_code || addExtraQueryParameters(parameters, {
      [RETURN_SPA_CODE]: "1"
  ...
  createLogoutUrlQueryString(request) {
    ...
    }(parameters, request.logoutHint), request.extraQueryParameters && addExtraQueryParameters(parameters, request.extraQueryParameters), 
    this.config.authOptions.instanceAware && addInstanceAware(parameters), mapToQueryString(parameters, this.config.authOptions.encodeExtraQueryParams, request.extraQueryParameters);
  }
}

/*! @azure/msal-common v15.13.3 2025-12-04 */ class RefreshTokenClient extends BaseClient {
  constructor(configuration, performanceClient) {
  ...
  async acquireTokenWithCachedRefreshToken(request, foci) {
    ...
    if (refreshToken.expiresOn && isTokenExpired(refreshToken.expiresOn, request.refreshTokenExpirationOffsetSeconds || 300)) {
      ...
      }, request.correlationId), createInteractionRequiredAuthError("refresh_token_expired");
    }
    ...
    try {
      ...
      if (e instanceof InteractionRequiredAuthError && (this.performanceClient?.addFields({
        rtExpiresOnMs: Number(refreshToken.expiresOn)
      }, request.correlationId), "bad_token" === e.subError)) {
  ...
  async executeTokenRequest(request, authority) {
    ...
    const queryParametersString = this.createTokenQueryParameters(request), endpoint = UrlString.appendQueryString(authority.tokenEndpoint, queryParametersString), requestBody = await invokeAsync(this.createTokenRequestBody.bind(this), PerformanceEvents_RefreshTokenClientCreateTokenRequestBody, this.logger, this.performanceClient, request.correlationId)(request), headers = this.createTokenRequestHeaders(request.ccsCredential), thumbprint = getRequestThumbprint(this.config.authOptions.clientId, request);
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ class SilentFlowClient extends BaseClient {
  constructor(configuration, performanceClient) {
    super(configuration, performanceClient);
  }
...
/*! @azure/msal-common v15.13.3 2025-12-04 */ function getStandardAuthorizeRequestParameters(authOptions, request, logger, performanceClient) {
  ...
  addRedirectUri(parameters, request.redirectUri), addCorrelationId(parameters, correlationId), 
  function(parameters, responseMode) {
    parameters.set("response_mode", responseMode || ResponseMode_QUERY);
  }(parameters, request.responseMode), addClientInfo(parameters), request.prompt && function(parameters, prompt) {
    ...
    if (request.sid && request.prompt === PromptValue_NONE) {
      ...
      /*! @azure/msal-common v15.13.3 2025-12-04 */ (request.account);
      if (accountLoginHintClaim && request.domainHint && (logger.warning('AuthorizationCodeClient.createAuthCodeUrlQueryString: "domainHint" param is set, skipping opaque "login_hint" claim. Please consider not passing domainHint'), 
      ...
      request.loginHint && (logger.verbose("createAuthCodeUrlQueryString: No account, adding login_hint from request"), 
      addLoginHint(parameters, request.loginHint), addCcsUpn(parameters, request.loginHint));
    }
  } else {
  ...
  (request.claims || authOptions.clientCapabilities && authOptions.clientCapabilities.length > 0) && addClaims(parameters, request.claims, authOptions.clientCapabilities), 
...
class ServerTelemetryManager {
  constructor(telemetryRequest, cacheManager) {
    ...
    this.wrapperSKU = telemetryRequest.wrapperSKU || Constants$1.EMPTY_STRING, this.wrapperVer = telemetryRequest.wrapperVer || Constants$1.EMPTY_STRING, 
  ...
  static maxErrorsToSend(serverTelemetryEntity) {
    ...
    for (i = 0; i < errorCount; i++) {
      const apiId = serverTelemetryEntity.failedRequests[2 * i] || Constants$1.EMPTY_STRING, correlationId = serverTelemetryEntity.failedRequests[2 * i + 1] || Constants$1.EMPTY_STRING, errorCode = serverTelemetryEntity.errors[i] || Constants$1.EMPTY_STRING;
  ...
  getRegionDiscoveryFields() {
    ...
    return regionDiscoveryFields.push(this.regionUsed || Constants$1.EMPTY_STRING), 
    regionDiscoveryFields.push(this.regionSource || Constants$1.EMPTY_STRING), regionDiscoveryFields.push(this.regionOutcome || Constants$1.EMPTY_STRING), 
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class Deserializer {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ const HttpMethod_GET = "get", HttpMethod_POST = "post", ProxyStatus = {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ const name = "@azure/msal-node";
...
class HttpClient {
  constructor(proxyUrl, customAgentOptions, loggerOptions) {
    this.networkRequestViaProxy = (httpMethod, destinationUrlString, options, timeout) => {
      const destinationUrl = new URL(destinationUrlString), proxyUrl = new URL(this.proxyUrl), headers = options?.headers || {}, tunnelRequestOptions = {
      ...
      this.customAgentOptions && Object.keys(this.customAgentOptions).length && (tunnelRequestOptions.agent = new http.Agent(this.customAgentOptions));
      ...
      return new Promise((resolve, reject) => {
        ...
        timeout && request.on("timeout", () => {
          ...
          request.destroy(), reject(new Error(`Request time out after ${timeout}ms`));
          ...
          socket.on("data", chunk => {
            ...
            const parsedHeaders = Object.fromEntries(entries), networkResponse = NetworkUtils.getNetworkResponse(parsedHeaders, this.parseBody(httpStatusCode, statusMessage, parsedHeaders, body), httpStatusCode);
            this.shouldDestroyRequest(httpStatusCode, networkResponse) && request.destroy(), 
          ...
          this.logUrlWithPiiAwareness("Destination URL", destinationUrlString), this.logUrlWithPiiAwareness("Proxy URL", this.proxyUrl), 
          this.logger.error(`HttpClient - Method: ${httpMethod}`, ""), this.logger.errorPii(`HttpClient - Headers: ${JSON.stringify(headers)}`, ""), 
          request.destroy(), reject(new Error(chunk.toString()));
        });
      });
    }, this.networkRequestViaHttps = (httpMethod, urlString, options, timeout) => {
      const isPostRequest = httpMethod === HttpMethod_POST, body = options?.body || "", url = new URL(urlString), headers = options?.headers || {}, customOptions = {
        method: httpMethod,
        headers: headers,
        ...NetworkUtils.urlToHttpOptions(url)
      };
      return this.customAgentOptions && Object.keys(this.customAgentOptions).length && (customOptions.agent = new https.Agent(this.customAgentOptions)), 
      isPostRequest ? customOptions.headers = {
        ...
        isPostRequest && request.write(body), timeout && request.on("timeout", () => {
          ...
          response.on("data", chunk => {
            ...
            const body = Buffer.concat([ ...data ]).toString(), parsedHeaders = headers, networkResponse = NetworkUtils.getNetworkResponse(parsedHeaders, this.parseBody(statusCode, statusMessage, parsedHeaders, body), statusCode);
            this.shouldDestroyRequest(statusCode, networkResponse) && request.destroy(), resolve(networkResponse);
          });
        }), request.on("error", chunk => {
          this.logger.error(`HttpClient - HTTPS request error: ${chunk.toString()}`, ""), 
          this.logUrlWithPiiAwareness("URL", urlString), this.logger.error(`HttpClient - Method: ${httpMethod}`, ""), 
          this.logger.errorPii(`HttpClient - Headers: ${JSON.stringify(headers)}`, ""), request.destroy(), 
    ...
    }, this.shouldDestroyRequest = (statusCode, networkResponse) => (statusCode < HttpStatus_SUCCESS_RANGE_START || statusCode > HttpStatus_SUCCESS_RANGE_END) && !(networkResponse.body && "object" == typeof networkResponse.body && "error" in networkResponse.body && networkResponse.body.error === Constants_AUTHORIZATION_PENDING), 
    this.proxyUrl = proxyUrl || "", this.customAgentOptions = customAgentOptions || {}, 
    this.logger = new Logger(loggerOptions || {}, name, "3.8.4"), this.isPiiEnabled = this.logger.isPiiLoggingEnabled();
  }
  async sendGetRequestAsync(url, options, timeout) {
    return this.proxyUrl ? this.networkRequestViaProxy(HttpMethod_GET, url, options, timeout) : this.networkRequestViaHttps(HttpMethod_GET, url, options, timeout);
  }
  async sendPostRequestAsync(url, options) {
    return this.proxyUrl ? this.networkRequestViaProxy(HttpMethod_POST, url, options) : this.networkRequestViaHttps(HttpMethod_POST, url, options);
  }
}

/*! @azure/msal-node v3.8.4 2025-12-04 */ const NodeAuthErrorMessage_invalidLoopbackAddressType = {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ const DEFAULT_AUTH_OPTIONS = {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class GuidGenerator {
  generateGuid() {
    return v4();
  }
  isGuid(guid) {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class EncodingUtils {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class HashUtils {
  sha256(buffer) {
    return crypto$1.createHash(Hash_SHA256).update(buffer).digest();
  }
}

/*! @azure/msal-node v3.8.4 2025-12-04 */ class PkceGenerator {
  constructor() {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class CryptoProvider {
  constructor() {
...
class NodeStorage extends CacheManager {
  ...
  generateCredentialKey(credential) {
    return function(credential) {
      ...
      return [ credential.homeAccountId, credential.environment, credential.credentialType, familyId, credential.realm || "", credential.target || "", credential.requestedClaimsHash || "", scheme ].join(CACHE_KEY_SEPARATOR).toLowerCase();
    }(credential);
  }
  generateAccountKey(account) {
    return function(account) {
      const homeTenantId = account.homeAccountId.split(".")[1];
      return [ account.homeAccountId, account.environment, homeTenantId || account.tenantId || "" ].join(CACHE_KEY_SEPARATOR).toLowerCase();
    }(account);
  }
  getAccountKeys() {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ const defaultSerializedCache_Account = {}, defaultSerializedCache_IdToken = {}, defaultSerializedCache_AccessToken = {}, defaultSerializedCache_RefreshToken = {}, defaultSerializedCache_AppMetadata = {};
...
function requireSafeBuffer() {
  return hasRequiredSafeBuffer || (hasRequiredSafeBuffer = 1, function(module, exports$1) {
    ...
    Buffer.from && Buffer.alloc && Buffer.allocUnsafe && Buffer.allocUnsafeSlow ? module.exports = buffer : (copyProps(buffer, exports$1), 
    exports$1.Buffer = SafeBuffer), SafeBuffer.prototype = Object.create(Buffer.prototype), 
...
function requireRe() {
  return hasRequiredRe || (hasRequiredRe = 1, function(module, exports$1) {
    const {MAX_SAFE_COMPONENT_LENGTH: MAX_SAFE_COMPONENT_LENGTH, MAX_SAFE_BUILD_LENGTH: MAX_SAFE_BUILD_LENGTH, MAX_LENGTH: MAX_LENGTH} = requireConstants(), debug = requireDebug(), re = (exports$1 = module.exports = {}).re = [], safeRe = exports$1.safeRe = [], src = exports$1.src = [], safeSrc = exports$1.safeSrc = [], t = exports$1.t = {};
    let R = 0;
    ...
    exports$1.tildeTrimReplace = "$1~", createToken("TILDE", `^${src[t.LONETILDE]}${src[t.XRANGEPLAIN]}$`), 
    createToken("TILDELOOSE", `^${src[t.LONETILDE]}${src[t.XRANGEPLAINLOOSE]}$`), createToken("LONECARET", "(?:\\^)"), 
    createToken("CARETTRIM", `(\\s*)${src[t.LONECARET]}\\s+`, !0), exports$1.caretTrimReplace = "$1^", 
    ...
    exports$1.comparatorTrimReplace = "$1$2$3", createToken("HYPHENRANGE", `^\\s*(${src[t.XRANGEPLAIN]})\\s+-\\s+(${src[t.XRANGEPLAIN]})\\s*$`), 
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class UsernamePasswordClient extends BaseClient {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ function getAuthCodeRequestUrl(config, authority, request, logger) {
  ...
  return addLibraryInfo(parameters, {
    ...
    cpu: process.arch || "",
    os: process.platform || ""
  }), config.auth.protocolMode !== ProtocolMode_OIDC && addApplicationTelemetry(parameters, config.telemetry.application), 
  ...
  }(parameters, request.codeChallenge, request.codeChallengeMethod), addExtraQueryParameters(parameters, request.extraQueryParameters || {}), 
  function(authority, requestParameters, encodeParams, extraQueryParameters) {
    const queryString = mapToQueryString(requestParameters, encodeParams, extraQueryParameters);
    return UrlString.appendQueryString(authority.authorizationEndpoint, queryString);
  }(authority, parameters, config.auth.encodeExtraQueryParams, request.extraQueryParameters);
}

/*! @azure/msal-node v3.8.4 2025-12-04 */ class ClientApplication {
  constructor(configuration) {
    ...
    }(configuration), this.cryptoProvider = new CryptoProvider, this.logger = new Logger(this.config.system.loggerOptions, name, "3.8.4"), 
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class LoopbackClient {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class DeviceCodeClient extends BaseClient {
  constructor(configuration) {
    super(configuration);
  }
  async acquireToken(request) {
  ...
  createExtraQueryParameters(request) {
    const parameters = new Map;
    return request.extraQueryParameters && addExtraQueryParameters(parameters, request.extraQueryParameters), 
    mapToQueryString(parameters);
  }
  ...
  createQueryString(request) {
    ...
    request.extraQueryParameters && addExtraQueryParameters(parameters, request.extraQueryParameters), 
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ class PublicClientApplication extends ClientApplication {
  constructor(configuration) {
...
/*! @azure/msal-node v3.8.4 2025-12-04 */ process.env.ProgramData, process.env.ProgramFiles;
...
class AuthProvider {
  ...
  async getToken(request, forceAuthInteractive, deviceCodeCallback = null) {
    let authResponse;
    const account = this.account || await this.getAccount(), tokenRequest = {
      ...request,
      account: null
    };
...
const cachePlugin = CACHE_LOCATION => {
  (CACHE_LOCATION => {
    ...
    try {
      return fs.existsSync(CACHE_LOCATION) ? (console.info("[MMM-OneDrive] [AuthProvider cachePlugin] Cache file exists."), 
      ...
      !0) : (console.warn("[MMM-OneDrive] [AuthProvider cachePlugin] Cache file does not exist."), 
      !1);
    } catch (err) {
      return console.error("[MMM-OneDrive] [AuthProvider cachePlugin] Cache file is not accessible.", err), 
      !1;
    }
  })(CACHE_LOCATION);
  return {
...
}, internetStatusListener = new node_stream.EventEmitter, isOnline = async (timeout = 25e3) => {
...
class OneDrivePhotos extends events.EventEmitter {
  ...
  constructor(options) {
    ...
    this.getAuthProvider = () => authProviderInstance ? (this.log("Get AuthProvider from cache"), 
    authProviderInstance) : (this.log("Initializing AuthProvider"), this.#debug && (msalConfig.system.loggerOptions.logLevel = LogLevel.Trace), 
